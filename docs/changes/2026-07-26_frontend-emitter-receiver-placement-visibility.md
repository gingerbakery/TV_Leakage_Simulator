...
- Receiver 기준면은 component와 겹치지 않는 보라색으로 변경하고 저장
  상태의 투명도도 높였다.
- 공통 방향 화살표의 입체 shaft와 채워진 화살촉을 제거했다. 완전히
  불투명한 얇은 한 픽셀 선과 작은 열린 V자 촉만 사용하고, normal 표시
  길이의 28%만 그려 모델 형상을 가리지 않고 방향만 확인할 수 있게 했다.
- Wireframe에서는 저장된 CAD surface Emitter의 노란 면 불투명도를 16%로
  낮춰 발광면 표시는 유지하면서 원래 CAD 숨은 모서리를 가리지 않게 했다.
