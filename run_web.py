...
<body>
  <div class=\"app\">
    ...
    <main class=\"viewer-wrap\">
      <div class=\"viewer-head\">
        ...
        <div id=\"viewerTip\" class=\"tip\">Drag = rotate, Middle drag = rotate, Wheel = zoom, Right click = component menu, Right drag = pan, Shift/Alt+drag = roll, F/Double-click = fit view.</div>
      </div>
  ...
  <script>
    ...
    function deleteComponentObject(objectId) {{
      ...
      state.roiScopes = state.roiScopes
        .map(scope => ({{
          ...scope,
          components: scope.components.filter(component => component.componentId !== objectId),
        }}))
        .filter(scope => scope.components.length);
    ...
    function updateSelectionModeUI() {{
      ...
      if (state.gapTargetMode === 'face_gap') {{
        ...
        viewerTip.textContent = dragSelect
          ? 'Drag = local face 박스 선택, Ctrl+Drag = add/remove, Middle drag = rotate, Wheel = zoom, Right click = component menu, Right drag = pan, F = fit view.'
    ...
    function recomputeSelectedFaces() {{
      ...
      const nextFaces = new Set(uniqueSorted(merged.filter(faceIndex => !isFaceDeleted(faceIndex))));
      const changed = nextFaces.size !== state.selectedFaces.size
        || Array.from(nextFaces).some(faceIndex => !state.selectedFaces.has(faceIndex));
      state.selectedFaces = nextFaces;
      if (changed) invalidateDirectRayTraceResult();
      updateRoiStats();
      updateViewerMode();
      updateGapSelectionStats();
      drawViewer();
    ...
    function updateEmitterSelectionUI() {{
      ...
      if (isFaceSelecting) {{
        ...
        viewerTip.textContent = 'Drag = rotate, Middle drag = rotate, Wheel = zoom, Right click = component menu, Right drag = pan, Shift/Alt+drag = roll, F/Double-click = fit view.';
      }} else {{
    ...
    function updateReceiverSelectionUI() {{
      ...
      if (isReferenceSelecting) {{
        ...
        if (!state.emitterSelectionActive) viewerTip.textContent = 'Drag = rotate, Middle drag = rotate, Wheel = zoom, Right click = component menu, Right drag = pan, Shift/Alt+drag = roll, F/Double-click = fit view.';
      }} else {{
    ...
    roiBoxDragArmToggle.addEventListener('click', function () {{
      ...
      updateRoiBoxDragArmUI();
      if (state.roiBoxDragArmed) {{
        try {{
          snapCameraToNearestFrontBack();
        }} catch (err) {{
          console.error('ROI camera snap failed; box selection remains armed', err);
        }}
      }}
    }});
